...
(s/defmethod wire->internal :reorder-category :- internal.cart-events/ReorderCategoryEvent
  [{:keys [event-type shopping-id category-id event-id purchase-list-id] :as wire} :- wire.in.shopping-events/ReorderCategoryEvent
   moment :- s/Num
   user-id :- s/Uuid]
...
(s/defmethod wire->internal :reorder-item :- internal.cart-events/ReorderItemEvent
  [{:keys [event-type shopping-id new-category-id item-id event-id purchase-list-id] :as wire} :- wire.in.shopping-events/ReorderItemEvent
   moment :- s/Num
   user-id :- s/Uuid]
  (-> (assoc wire :event-type (keyword event-type)
                  :shopping-id (adapters.misc/string->uuid shopping-id)
...
(s/defmethod wire->internal :change-item :- internal.cart-events/ChangeItemEvent
  [{:keys [event-type shopping-id item-id event-id purchase-list-id] :as wire} :- wire.in.shopping-events/ChangeItemEvent
   moment :- s/Num
   user-id :- s/Uuid]
  (->
    (assoc wire :event-type (keyword event-type)
