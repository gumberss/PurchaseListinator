...
(s/defn get-items-by-item-id :- models.shopping-item-events/ShoppingItemEventCollection
  [item-id :- s/Uuid
   {:shopping-events/keys [main-db]}]
  {:item-id item-id
   :events (diplomat.db.shopping-events/get-by-item-id item-id main-db)})

(s/defn get-items-by-items :- [models.shopping-item-events/ShoppingItemEventCollection]
  [items-ids :- [s/Uuid]
   {:shopping-events/keys [main-db]}]
